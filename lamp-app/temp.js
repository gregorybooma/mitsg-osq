			...
			if (!correctEmpty){
				for (var i=0; i<correct.length; i++){
					if (correct[i]==j){
						...
						tableString = "<tr><td class='icon'><img src='pics/checkmark.png' alt='Correct'/></td><td>" + c[j] + " / " + d[j] + "</td> <td>"+q[j]+"</td><td><div style='float:right;margin-left:10px;'><a href='javascript:void(0)' title='" + r[j] + "'><img src='pics/info.png' border='0' align='absmiddle' /></a></div>"+ans+"</td><td/><td nowrap='nowrap'><img class='rateImg' src='pics/iconThumbsup.png' border='0' align='absmiddle' rel='" + q[j] + "' id='" + j + "+'/> <img class='rateImg' src='pics/iconThumbsdown.png' border='0' align='absmiddle' rel='" + q[j] + "' id='" + j + "-'/></td></tr>";	
			...
			if (!wrongEmpty && checkInWrong){
				for(var i=0; i<wrong.length; i++) {
					if (wrong[i]==j){
						ans = getAnswer(answer[j], j);
						userResponse = userAns[j];
						tableString = "<tr><td class='icon'><img src='pics/Xmark.png' alt='Wrong'/></td><td>" + c[j] + " / " + d[j] + "</td><td>"+q[j]+"</td><td><div style='float:right;margin-left:10px;'><a href='javascript:void(0)' title='" + r[j] + "'><img src='pics/info.png' border='0' align='absmiddle' /></a></div>"+ans+"</td><td>"+userResponse+"</td><td nowrap='nowrap'><img class='rateImg' src='pics/iconThumbsup.png' border='0' align='absmiddle' rel='" + q[j] + "' id='" + j + "+'/> <img class='rateImg' src='pics/iconThumbsdown.png' border='0' align='absmiddle' rel='" + q[j] + "' id='" + j + "-'/></td></tr>";
						$('#questionTable').append(tableString);
					}
				}
			}
			...
			for (var j=0; j<q.length; j++) {
				...
				if (!wrongP2Empty && checkInWrong){
					for(var i=0; i<wrongP2.length; i++) {
						if (wrongP2[i]==j){
							ans = getAnswer(answerP2[j], j);
							alert(j+userAns.length);
							//alert("Item " + j + " = " + userAnsP2[j+userAns.length] + ", and " + ans);
							userResponse = userAns[j];
							tableString = "<tr><td class='icon'><img src='pics/Xmark.png' alt='Wrong'/></td><td>" + c[j] + " / " + d[j] + "</td><td>"+q[j]+"</td><td><div style='float:right;margin-left:10px;'><a href='javascript:void(0)' title='" + r[j] + "'><img src='pics/info.png' border='0' align='absmiddle' /></a></div>"+ans+"</td><td>" + userAnsP2[j] + "</td><td nowrap='nowrap'><img class='rateImg' src='pics/iconThumbsup.png' border='0' align='absmiddle' rel='" + q[j] + "' id='" + j + "+'/> <img class='rateImg' src='pics/iconThumbsdown.png' border='0' align='absmiddle' rel='" + q[j] + "' id='" + j + "-'/></td></tr>";
							$('#questionTable').append(tableString);
						}
					}
